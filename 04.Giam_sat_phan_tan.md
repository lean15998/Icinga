...
<ul>
  ...
  <li> Một node master có thể kết hợp các kiểm tra đã thực thi từ các node con thành phần phụ trợ và thông báo.
 </ul>
</ul>
...
<ul>
  ...
  <li> Một node satellite có thể nhận cấu hình cho các máy chủ / dịch vụ, v.v. từ node cha.
...
<ul>
  <ul>
    <li> Một node agent sẽ chạy các kiểm tra được cấu hình của chính nó hoặc nhận các sự kiện thực thi lệnh từ node cha.
  </ul>
</ul>

- Client có thể là master, satellite hoặc agent. Nó thường yêu cầu một cái gì đó từ node master hoặc node cha.
...
<img src = "https://github.com/lean15998/Icinga/blob/main/image/4.02.png">
 ...
 - Cấu hình các node satellite có node master là zone parent:
...
<ul>
  ...
  <li> Tạo chứng chỉ cho node này được ký bởi khóa CA.
...
- Tạo một vé trên node master(master1) cho agent(agent1):
 ```sh
[root@master1 /]# icinga2 pki ticket --cn "agent1"
...
| Master/Satellite endpoint host |	Bắt buộc nếu agent cần kết nối với master/satellite. Địa chỉ IP của điểm cuối chính hoặc FQDN. Thông tin này được bao gồm trong cấu hình Endpoint trong tệp zones.conf. |
| Master/Satellite endpoint port	| Tùy chọn nếu đại lý cần kết nối với master/satellite. Cổng lắng nghe của thiết bị đầu cuối chính. Thông tin này được bao gồm trong Endpointcấu hình đối tượng. |
| Add more master/satellite endpoints	| Không bắt buộc. Nếu bạn đã định cấu hình nhiều node master/satellite, hãy thêm chúng vào đây. |
| Parent Certificate information	| Yêu cầu. Xác minh rằng máy chủ kết nối thực sự là node master được yêu cầu. |
| Request ticket	| Không bắt buộc. Thêm vé được tạo trên node master. |
| API bind host	| Không bắt buộc. Cho phép chỉ định địa chỉ mà ApiListener bị ràng buộc. Chỉ dành cho mục đích sử dụng nâng cao. |
...
| Accept config	| Không bắt buộc. Liệu node này có chấp nhận đồng bộ cấu hình từ node chính (bắt buộc đối với chế độ đồng bộ cấu hình ) hay không. Vì lý do bảo mật , điều này được mặc định thành n. |
| Accept commands	| Không bắt buộc. Liệu node này có chấp nhận thông báo thực thi lệnh từ node chính hay không (bắt buộc đối với chế độ điểm cuối lệnh ). Vì lý do bảo mật , điều này được mặc định thành n. |
...
| Disable conf.d	| Không bắt buộc. Cho phép vô hiệu hóa việc bao gồm thư mục conf.dchứa cấu hình ví dụ cục bộ. Khách hàng nên truy xuất cấu hình của họ từ node cha hoặc hoạt động như cầu nối thực thi điểm cuối lệnh. Mặc định là y. |
...
- Chế độ này node Icinga2 thực hiện các lệnh từ xa trên một endpoint được chỉ định. Cấu hình đối tượng host / service được đặt trên master/satellite và agent chỉ cần các định nghĩa CheckCommand object có sẵn.
...
<ul>
  ...
  <li> Không cần nhật ký phát lại cho node con.
...
<ul>
  <ul>
  <li> Nếu node con không được kết nối, không có kiểm tra nào được thực hiện nữa.
...
- Để đảm bảo rằng tất cả các node có liên quan đều chấp nhận cấu hình và lệnh, bạn cần phải định cấu hình phân cấp Zone và Endpoint trên tất cả các node.
...
<ul>
  ...
  <li> Node satellite1 chấp nhận cấu hình và điền vào kho lưu trữ cấu hình vùng cục bộ với các tệp cấu hình đã nhận.
